...
- **`diag_eq_traces.py`**: Generates a 4-panel diagnostic visualization of two-start equilibration traces (random vs. ordered start) at representative temperatures. Pre-screens seeds at low temperature to identify and highlight trajectories that become stuck in metastable states. Results are saved as a PNG figure in the output directory.
