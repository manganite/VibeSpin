"""
Domain growth analysis for the 2D XY model.

Quenches from a disordered state to T < T_BKT and records:
...
def compute_domain_size(sim: XYSimulation) -> float:
...
def compute_vortex_density(sim: XYSimulation) -> float:
...
def compute_correlation_length(sim: XYSimulation) -> float:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description='2D XY Model Domain Growth Analysis')
    ...
    logger.info(f"XY domain growth analysis (L={L}, T={T:.3f} < T_BKT={T_BKT})")
    ...
    title = f'2D XY Domain Growth — $T = {T}$ ($< T_{{BKT}} \\approx {T_BKT}$), $L = {L}$'
